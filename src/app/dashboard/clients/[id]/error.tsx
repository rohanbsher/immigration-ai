...
const log = createLogger('client-detail-error');
...
export default function ClientDetailError({ error, reset }: ClientDetailErrorProps) {
  useEffect(() => {
    log.logError('Client Detail Error', error, { digest: error.digest });

    Sentry.captureException(error, { tags: { area: 'client-detail' } });
  }, [error]);
      ...
      <Card className="w-full max-w-lg">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-muted-foreground">
            We were unable to load the client details. The client record may
